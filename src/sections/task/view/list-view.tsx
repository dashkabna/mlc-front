...
import { order, task } from 'src/types/types';
...
const TABLE_HEAD = [
  { id: 'index', label: '№' },
  { id: 'id', label: 'Id' },
  { id: 'title', label: 'Гарчиг' },
  { id: 'description', label: 'Дэлгэрэнгүй' },
...
export default function ListView() {
  ...
  const getList = async () => {
    ...
    const response = await axios.get(
      `/api/post?url=${getBaseUrl(BaseUrlTypes.ENUM_HOST_BASE_URI) + endpoints.task}`
    );
  ...
  const handleDeleteRow = useCallback(async (id: string) => {
    try {
      await axios.delete(
        `/api/post?url=${getBaseUrl(BaseUrlTypes.ENUM_HOST_BASE_URI) + endpoints.taskId(id)}`
      );
      getList();
    } catch (error) {
